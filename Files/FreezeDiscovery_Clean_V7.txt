<#
  FreezeDiscovery_Clean_V7.ps1
  - PS 5.1 / ISE safe
  - Adds Application log Event ID 1001 reporting (same process used for System 153 via Get-WinEvent)
  - Merges Application 1001 events into SystemEvents_48h.csv (same CSV file)
  - Preserves V6 fixes: safe writers, robust fltmc parsing, guaranteed A2 artifacts, TargetPath empty-safe
#>
...
# ==========================================================
# Safe writers (fix empty-string binding issues)
# ==========================================================
function Write-Text {
    param(
...
function Parse-FltmcInstances {
    ...
    foreach ($r in $rows) {
        ...
        # Modern:
        # With Volume: Filter, Volume, Altitude, Instance, Frame, SprtFtrs, VlStatus (>=7)
        # No Volume:   Filter, Altitude, Instance, Frame, SprtFtrs, VlStatus        (=6)
        # Older:
        # With Volume: Filter, Volume, Altitude, Instance, Frame, Flags             (=6)
        # No Volume:   Filter, Altitude, Instance, Frame, Flags                     (=5)
        ...
        elseif ($p.Count -eq 6) {
            ...
            if ($looksLikeVolume) {
                # older with-volume; store Flags in SprtFtrs
                $volume = $p[1]
                $alt    = $p[2]
                $inst   = $p[3]
                $frame  = $p[4]
        ...
        elseif ($p.Count -eq 5) {
            # older no-volume; store Flags in SprtFtrs
            $volume = $null
            $alt    = $p[1]
            $inst   = $p[2]
            $frame  = $p[3]
...
# ==========================================================
# A2: Filters-on-target-volume summary (TargetPath empty-safe)
# ==========================================================
...
# ==========================================================
# A3 signals (UPDATED): add Application ID 1001 and merge into SystemEvents_48h.csv
#   - Same process as System 153 reporting: Get-WinEvent with FilterHashtable, select fields, count, export
# ==========================================================
function Get-EventSignals {
    ...
    # System IDs (same as before)
    $sysIds = 41,107,137,129,153

    # Application ID to add (per request)
    $appIds = 1001
    ...
    try {
        $sysEv = Get-WinEvent -FilterHashtable @{ LogName="System"; StartTime=$since; Id=$sysIds } -ErrorAction SilentlyContinue
    } catch { $sysEv = @() }

    try {
        $appEv = Get-WinEvent -FilterHashtable @{ LogName="Application"; StartTime=$since; Id=$appIds } -ErrorAction SilentlyContinue
    } catch { $appEv = @() }

    # Normalize to a unified event shape for CSV merge
    ...
    $allRows = @($sysRows + $appRows)

    # Sort newest first for ticket friendliness
    $allRows = @($allRows | Sort-Object -Property TimeCreated -Descending)

    return [pscustomobject]@{
        LookbackHours          = $LookbackHours

        # System-derived counts (same reasoning as before)
        ...
        # New Application-derived count
        Application1001Count   = @($appEv | Where-Object { $_.Id -eq 1001 }).Count

        # Combined events (System + Application 1001) for CSV export
        Events                 = $allRows
    }
}
...
# ==========================================================
# A4 routing: classification + confidence + suspects
# ==========================================================
...
Write-JsonSafe -Path (Join-Path $bundle "TargetContext.json") -Value ([pscustomobject]@{ TargetPath = $TargetPath; PathType = $pathType })

# A3 signals (now includes Application 1001 merged into Events)
$eventSignals = Get-EventSignals -LookbackHours 48
Write-JsonSafe -Path (Join-Path $bundle "EventSignals.json") -Value $eventSignals

# Export merged events to the SAME file name (SystemEvents_48h.csv) per request
...
# ----------------------------------------------------------
# FLTMC + mapping + A2 (GUARANTEED artifacts)
# ----------------------------------------------------------
$miniFilters         = @()
$miniInstancesMapped = @()
$A2                  = $null
...
try {
    ...
    # Ensure these exist even on failure
    Write-JsonSafe -Path (Join-Path $bundle "MiniFilters.json")         -Value $miniFilters
    Write-JsonSafe -Path (Join-Path $bundle "MiniFilterInstances.json") -Value $miniInstancesMapped
}
finally {
    # GUARANTEE: A2 artifacts always written
    try {
        ...
        Write-Text -Path (Join-Path $bundle "FiltersOnTargetVolume.txt") -Content @(
            "FiltersOnTargetVolume Summary"
            "-----------------------------"
        ...
        Write-JsonSafe -Path (Join-Path $bundle "FiltersOnTargetVolume.json") -Value $null
...
# Human summary (include new Application1001Count)
$humanLines = @(
    ...
    ("  Application1001Count: {0}" -f $eventSignals.Application1001Count)
    ""
    "Routing (A4):"
...
# Ticket summary (mention Application 1001 and that CSV now includes it)
...
$ticketLines = @(
    ...
    ("- Application1001Count (Application 1001): {0}" -f $eventSignals.Application1001Count)
    ""
    ...
    "- EventSignals.json"
    "- SystemEvents_48h.csv   (includes System + Application(1001))"
