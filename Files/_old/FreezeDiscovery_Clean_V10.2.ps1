<#
FreezeDiscovery_Clean_V10_Registry.ps1
- PowerShell 5.1 / ISE safe
- Enterprise-hardened: StrictMode 2.0, safe writers, robust parsing, best-effort collector, guaranteed artifacts
- Registry-based OS info (no Win32_OperatingSystem CIM/WMI)
- WMI-free volume mapping (mountvol + QueryDosDevice)
- Optional ETW trace: WPR fileio + minifilter -> MinifilterTrace.etl + ETWTraceMeta.json

USAGE:
  .\FreezeDiscovery_Clean_V10_Registry.ps1 -TargetPath "C:\Some\Path" -CaseId "INC123"
  .\FreezeDiscovery_Clean_V10_Registry.ps1 -TargetPath "C:\Some\Path" -CaseId "INC123" -EnableEtwMinifilterTrace -EtwCaptureSeconds 90
#>
...
param(
    ...
    # Protect against rare Get-WinEvent stalls on busy/buggy systems
    [ValidateRange(5, 300)]
...
# ==========================================================
# Timeout wrapper (PS 5.1): prevents rare stalls in Get-WinEvent
# ==========================================================
function Invoke-WithTimeout {
    ...
    try {
        if (Wait-Job -Job $job -Timeout $TimeoutSec) {
            ...
            throw ("Timed out after {0} seconds." -f $TimeoutSec)
        }
    }
    finally {
...
# ==========================================================
# Registry-based OS info (no CIM/WMI)
# ==========================================================
function Get-OsInfoFromRegistry {
    try {
        ...
        return [pscustomobject]@{
            ...
            Version     = $(if ($cv.DisplayVersion) { $cv.DisplayVersion } elseif ($cv.ReleaseId) { $cv.ReleaseId } else { $cv.CurrentVersion })
...
function Get-VolumeGuidFromMountvol {
    param([Parameter(Mandatory=$true)][string]$DriveLetter) # e.g. "C:"

    # mountvol expects "C:\" format
    $driveRoot = $DriveLetter.TrimEnd('\') + "\"
    $r = Invoke-ExternalCommand -FilePath "mountvol.exe" -Arguments @($driveRoot, "/L") -TimeoutSeconds 10
    $txt = (($r.StdOut + "`n" + $r.StdErr) -split "`r?`n") | Where-Object { $_ -and $_.Trim() -ne "" }

    foreach ($line in $txt) {
        $t = $line.Trim()
        if ($t -match '^\\\\\?\\Volume\{[0-9A-Fa-f\-]+\}\\?$') {
            # normalize to \\?\Volume{GUID}
            return $t.TrimEnd('\')
        }
    }
...
function Get-VolumeMapNoWmi {
    Ensure-DosDeviceType
    ...
    foreach ($d in $drives) {
        ...
        try { $volGuid = Get-VolumeGuidFromMountvol -DriveLetter $dl } catch { $volGuid = $null }

        try {
            $sb = New-Object System.Text.StringBuilder 2048
            $rc = [DosDevice]::QueryDosDevice($dl, $sb, $sb.Capacity)
            if ($rc -gt 0) { $devPath = $sb.ToString() }
        } catch { $devPath = $null }
...
function Write-FiltersOnTargetVolumeArtifacts {
    ...
    return [pscustomobject]@{ TargetDrive=$drive; Lines=$inject; TopFilters=$top5; Summary=$summaryObj }
}

# ==========================================================
# A3 signals (System + Application 1002) with timeout protection
# ==========================================================
function Get-EventSignals {
    param([int]$LookbackHours = 48, [int]$TimeoutSec = 30)
    ...
    Write-Log -Level DEBUG -Message "Get-WinEvent(System) start..."
    try {
        $sysEv = Invoke-WithTimeout -TimeoutSec $TimeoutSec -ScriptBlock {
            param($since,$ids)
            Get-WinEvent -FilterHashtable @{ LogName="System"; StartTime=$since; Id=$ids } -ErrorAction SilentlyContinue
        } -ArgumentList @($since,$sysIds)
        if ($null -eq $sysEv) { $sysEv = @() }
    } catch {
        Write-Log -Level WARN -Message ("Get-WinEvent(System) failed/timeout: {0}" -f $_.Exception.Message)
        $sysEv = @()
    }

    Write-Log -Level DEBUG -Message "Get-WinEvent(Application 1002) start..."
    try {
        $appEv = Invoke-WithTimeout -TimeoutSec $TimeoutSec -ScriptBlock {
            param($since,$ids)
            Get-WinEvent -FilterHashtable @{ LogName="Application"; StartTime=$since; Id=$ids } -ErrorAction SilentlyContinue
        } -ArgumentList @($since,$appIds)
        if ($null -eq $appEv) { $appEv = @() }
    } catch {
        Write-Log -Level WARN -Message ("Get-WinEvent(Application) failed/timeout: {0}" -f $_.Exception.Message)
        $appEv = @()
    }
...
# System info (registry-based)
Write-Log -Message "Stage: SystemInfo (registry) start"
$sys = [pscustomobject]@{
    ComputerName = $env:COMPUTERNAME
    UserName     = $env:USERNAME
    OS           = $null
}
$sys.OS = Get-OsInfoFromRegistry
Write-JsonSafe -Path (Join-Path $script:BundlePath "SystemInfo.json") -Value $sys
Write-Log -Message "Stage: SystemInfo complete"

# Target context
Write-Log -Message "Stage: TargetContext start"
$pathType = "None"
if ($TargetPath) {
    if ($TargetPath -like "\\*") { $pathType = "UNC" }
    elseif ($TargetPath -match '^[A-Za-z]:') { $pathType = "Local" }
}
Write-JsonSafe -Path (Join-Path $script:BundlePath "TargetContext.json") -Value ([pscustomobject]@{
    TargetPath = $TargetPath
    PathType   = $pathType
})
Write-Log -Message "Stage: TargetContext complete"

# A3 Event signals
Write-Log -Message "Stage: EventSignals start"
$eventSignals = Get-EventSignals -LookbackHours $LookbackHours -TimeoutSec $WinEventTimeoutSec
Write-JsonSafe -Path (Join-Path $script:BundlePath "EventSignals.json") -Value $eventSignals

Write-Log -Message "Stage: SystemEvents CSV write"
...
Write-Log -Message "Stage: EventSignals complete"

# GlobalProtect signals
Write-Log -Message "Stage: GlobalProtectSignals start"
...
Write-Log -Message "Stage: GlobalProtectSignals complete"

# FLTMC + VolumeMap + A2
Write-Log -Message "Stage: MiniFilter capture start"
$miniFilters = @()
$miniInstancesMapped = @()
$A2 = $null
...
try {
    ...
    Write-Log -Message "Stage: VolumeMap (no WMI) start"
    $volumeMap = Get-VolumeMapNoWmi
    ...
    Write-Log -Level WARN -Message ("MiniFilter capture failed (best-effort). Error: {0}" -f $_.Exception.Message)
    ...
    try {
        Write-Log -Message "Stage: A2 FiltersOnTargetVolume start"
        ...
        Write-Log -Message "Stage: A2 complete"
    }
    catch {
        Write-Log -Level WARN -Message ("A2 summary failed. Error: {0}" -f $_.Exception.Message)
...
Write-Log -Message "Stage: MiniFilter capture complete"

# A4 routing
Write-Log -Message "Stage: A4 Routing start"
$topFilters = @()
...
if ($null -eq $routing) {
    $routing = [pscustomobject]@{
        Queue="Unknown"; Confidence="Low"; Score=0; Reason="Routing not computed"; Suspects=@(); Signals=[pscustomobject]@{ PathType=$pathType }
    }
}
...
Write-Log -Message "Stage: A4 Routing complete"

# ETW meta ALWAYS has Success (StrictMode-safe)
...
if ($EnableEtwMinifilterTrace) {
    Write-Log -Message ("Stage: ETW (WPR fileio+minifilter) start ({0}s)" -f $EtwCaptureSeconds)
    $etwMeta = Start-WprMinifilterTrace -BundlePath $script:BundlePath -CaptureSeconds $EtwCaptureSeconds
    Write-Log -Message ("Stage: ETW complete. Success={0}" -f $etwMeta.Success)
}
Write-JsonSafe -Path (Join-Path $script:BundlePath "ETWTraceMeta.json") -Value $etwMeta

# Human summary
Write-Log -Message "Stage: Summary_Human start"
$humanLines = @(
...
Write-Log -Message "Stage: Summary_Human complete"

# Ticket summary
Write-Log -Message "Stage: TicketSummary start"
$ticketLines = @(
