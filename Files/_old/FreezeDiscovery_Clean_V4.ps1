...
function Write-JsonSafe {
    ...
    # Always write a JSON file even when Value is null
    if ($null -eq $Value) {
...
function Get-FltmcRaw {
    ...
    # Normalize external output to string[]
...
function Parse-FltmcInstances {
    ...
    foreach ($r in $rows) {
        ...
        # Your real-world shapes:
        # A) With Volume: Filter, Volume, Altitude, Instance, Frame, Flags  (Count >= 6)
        # B) No Volume:   Filter, Altitude, Instance, Frame, Flags          (Count == 5)
...
# Guarantee artifacts (routing + suspects + classification)
if ($null -eq $routing) {
