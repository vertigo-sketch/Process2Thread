<#
AutoCaptureAnalyze_Dump_V05.ps1
...
- Updated evidence classification to include: Defender + CrowdStrike + SecurityDetected
- Updated escalation rule to also escalate on: Medium evidence + SecurityDetected + no IoSyscall evidence
#>
...
# Evidence signatures to spot in cdb output (expanded in V05)
$EvidenceIndicators = @(
  # I/O syscalls
  "ntdll!NtReadFile",
  "ntdll!NtCreateFile",

  # Filter stack / OneDrive
  "FLTMGR!",
  "cldflt!",

  # Security minifilters / drivers in stack evidence (NEW)
  "WdFilter!",
  "CSAgent!"
)
...
# NEW in V05: Rich evidence classification
function Classify-Evidence {
    ...
    # Strength model:
    # Strong: IO syscall + any filter involvement (FilterMgr or Cloud or Security)
    # Medium: IO syscall or filter involvement (but not both)
    # Weak: none
    $strength = "Weak"
...
# ============================================================
# Escalation logic (updated in V05)
# ============================================================
function Should-Escalate {
    ...
    # Rule 2 (NEW): Medium + Security detected + no I/O syscall -> escalate (more robust)
...
function Maybe-EscalateToFull {
    ...
    # Evidence insufficient -> capture 1 Full dump and analyze it
