...
# =========================================
# A3: Event + GlobalProtect signals (used by A4 too)
# =========================================
...
# =========================================
# A4: Known filter classification + confidence score (0-100)
# =========================================
function Classify-FilterName {
    ...
    if ($FilterName -match "OneDrive|CldFlt") { return "Microsoft OneDrive/Cloud Files" }

    # Common security/EDR patterns (heuristic)
    if ($FilterName -match "Crowd|Falcon") { return "EDR (CrowdStrike)" }
    if ($FilterName -match "Sentinel") { return "EDR (SentinelOne)" }
    if ($FilterName -match "Carbon|Cb") { return "EDR (VMware Carbon Black)" }
    ...
    # Backup / file indexing (heuristic)
...
function Score-RoutingConfidence {
    ...
    $gpActive = $false
    $smbActive = $false
    if ($GpSignals -and ($GpSignals.ServiceRunning -or $GpSignals.AdapterUp)) { $gpActive = $true }
    if ($GpSignals -and ($GpSignals.SmbConnectionCount -gt 0)) { $smbActive = $true }
...
function Compute-RoutingA4 {
    ...
    # Classify top suspects (TopFilters list from A2)
    $suspects = @()
    if ($TopFilters) {
    ...
    # Vendor-friendly reason expansion
...
function Write-SuspectFiltersTxt {
    ...
    if ($Routing.Suspects -and $Routing.Suspects.Count -gt 0) {
        ...
        $lines.Add("No suspect filters were identified (no A2 top filters available).")
    }
...
# A4 routing decision (includes score + suspects)
$topFilters = @()
...
# Also write classification list as a separate JSON for vendors
