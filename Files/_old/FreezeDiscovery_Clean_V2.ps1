...
function Write-JsonSafe {
    ...
    if ($null -eq $Value) {
        @{ Notice="Value was null"; Time=(Get-Date).ToString("o") } |
            ConvertTo-Json -Depth 3 | Out-File -FilePath $Path -Encoding UTF8 -Force
        return
    }
...
function Get-FltmcRaw {
    ...
    # External command output is typically string[]; @() preserves shape for 1 or many lines.
...
# =========================================
# A4: Classification + confidence score + suspects
# =========================================
...
# Always write a CSV with headers
$evForCsv = @($eventSignals.Events)
if (-not $evForCsv -or $evForCsv.Count -eq 0) {
    $evForCsv = @([pscustomobject]@{
        TimeCreated      = $null
        Id               = $null
        ProviderName     = $null
...
$evForCsv | Export-Csv (Join-Path $bundle "SystemEvents_48h.csv") -NoTypeInformation -Encoding UTF8 -Force
...
# Force artifact writes (ensures files exist even when suspects are empty)
try {
    if ($null -eq $routing) {
    ...
    Write-Text -Path (Join-Path $bundle "FilterClassification_WARNING.txt") -Content @(
        "Failed to write FilterClassification.json"
        ("Error: {0}" -f $_.Exception.Message)
    )
}
