...
Historical changes:
- V31: Fix CDB ExitCode=-2147024809 (0x80070057 invalid parameter) by quote-safe argument passing
       * Build CDB args as a single quoted string; capture stdout/stderr even when -logo log is missing [1](https://www.ionos.com/digitalguide/server/configuration/fixing-0x80070057-error/)
       * Record CDB success/failure + exit code (dec/hex) in analysis JSON
       * Keep ProcDump retries -64/-wer [2](https://icapitalnetwork-my.sharepoint.com/personal/jmiller_icapitalnetwork_com/_layouts/15/Doc.aspx?action=edit&mobileredirect=true&wdorigin=Sharepoint&DefaultItemOpen=1&sourcedoc={a8c8915f-6b87-4232-aba2-f8b09a805634}&wd=target(/Projects/process and handles.one/)&wdpartid={5276727a-0db4-44e0-a227-ca5b28bf705e}{1}&wdsectionfileid={fe5d4ba1-733f-4984-bc79-7a2c5154083e})
#>
...
function Capture-Dump {
    ...
    # ProcDump -wer [2](https://icapitalnetwork-my.sharepoint.com/personal/jmiller_icapitalnetwork_com/_layouts/15/Doc.aspx?action=edit&mobileredirect=true&wdorigin=Sharepoint&DefaultItemOpen=1&sourcedoc={a8c8915f-6b87-4232-aba2-f8b09a805634}&wd=target(/Projects/process and handles.one/)&wdpartid={5276727a-0db4-44e0-a227-ca5b28bf705e}{1}&wdsectionfileid={fe5d4ba1-733f-4984-bc79-7a2c5154083e})
...
# ---------------------------
# cdb analysis (quote-safe + stdout/stderr capture)
# ---------------------------
function Analyze-Dump {
    param([string]$DumpPath)

    $cdbLog = $DumpPath -replace '\.dmp$','.cdb.log'
    $cdbOut = $DumpPath -replace '\.dmp$','.cdb.stdout.log'
    $cdbErr = $DumpPath -replace '\.dmp$','.cdb.stderr.log'

    # command script kept simple; quoting handled by wrapping whole -c argument
    $cmd = '.symfix; .reload; !analyze -hang; ~* kb; q'

    # Build a single argument string with explicit quoting to avoid 0x80070057 invalid parameter
    $argLine = ('-z "{0}" -c "{1}" -logo "{2}"' -f $DumpPath, $cmd, $cdbLog)

    Write-Log "CDB START Dump=$DumpPath Log=$cdbLog Args=$argLine"
    $p = Start-Process -FilePath $CdbPath `
        -ArgumentList $argLine `
        -WindowStyle Hidden `
        -PassThru `
        -Wait `
        -RedirectStandardOutput $cdbOut `
        -RedirectStandardError $cdbErr

    $logExists = (Test-Path $cdbLog)
    $hex = Convert-ExitCodeHex -Code ([int]$p.ExitCode)
    Write-Log "CDB END ExitCode=$($p.ExitCode) ExitHex=$hex LogExists=$logExists StdOut=$cdbOut StdErr=$cdbErr"

    if (-not $logExists) {
        $errEx = Get-FileExcerpt -Path $cdbErr -MaxLines 40
        $outEx = Get-FileExcerpt -Path $cdbOut -MaxLines 40
        if ($errEx) { Write-Log ("CDB STDERR EXCERPT:`n{0}" -f $errEx) }
        if ($outEx) { Write-Log ("CDB STDOUT EXCERPT:`n{0}" -f $outEx) }
...
# MAIN LOOP
# ---------------------------
Write-Log "=== AutoCapture V31 START === ScriptRoot=$ScriptRoot DumpRoot=$DumpRoot DumpProfile=$DumpProfile"
Write-Status "AutoCapture V31 running (Admin OK). Logs: $LogFile" Cyan
...
do {
    ...
    foreach ($proc in Get-Process) {
        ...
        try {
            ...
            if ($cdbResult -and $cdbResult.Success -eq $true) {
