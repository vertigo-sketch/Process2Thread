<#
AutoCaptureAnalyze_Dump_V03.ps1
...
- Added top-of-file historical change log (this block)
...
function Get-ProcDumpDumpSwitch {
    ...
    switch ($Profile) {
        ...
        "MiniWithHandles" { return "-mp" }  # default minimal (smaller than full; keeps useful context)
        "Mini"            { return "" }     # smallest/default (no explicit dump-type switch)
    }
}
...
function Capture-DumpSeries {
    ...
    # ProcDump multi-dump series: -n (count) and -s (seconds) [1](https://learn.microsoft.com/en-us/sysinternals/downloads/sysinternals-suite)
...
function Analyze-Dump {
    ...
    # Offline symbols by default (avoids proxy/network stalls); online optional.
    $symPath = $SymCache
    if ($OnlineSyms) {
