<#
  FreezeDiscoveryClean_V6.ps1
  - PS 5.1 / ISE safe
  - Robust fltmc instances parsing (supports columns: Frame, SprtFtrs, VlStatus)
  - Preserves extra instance columns + RawLine (Option A)
  - Safe writers (no empty-string binding failures)
  - GUARANTEES FiltersOnTargetVolume.txt/.json exist even when fltmc fails
  - Fix: TargetPath empty string no longer breaks A2 / minifilter collection
#>
...
# ==========================================================
# Normalize Inputs (FIX: empty TargetPath becomes $null)
# ==========================================================
...
#   FIX: allow TargetPath to be null/empty (still writes artifacts)
# ==========================================================
...
function Write-FiltersOnTargetVolumeArtifacts {
    param(
        ...
        # FIX: allow empty/null target path (so caller can pass $null safely)
        [AllowNull()]
        [AllowEmptyString()]
        [string]$TargetPath,
...
try {
    ...
    # GUARANTEE: A2 artifacts always written (FIX: pass TargetPath which may be $null safely)
    try {
