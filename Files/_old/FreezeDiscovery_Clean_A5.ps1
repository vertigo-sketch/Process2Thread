...
# A4 Force artifact writes (ensures files exist even when suspects are empty)
try {
    ...
    # Ensure Suspects property exists and is not null
    if ($null -eq $routing.Suspects) {
        $routing | Add-Member -NotePropertyName Suspects -NotePropertyValue @() -Force
    }

    # Write routing file (you already see this one)
    Write-JsonSafe -Path (Join-Path $bundle "RoutingA4.json") -Value $routing

    # Write suspect list (you already see this one)
    Write-SuspectFiltersTxt -Path (Join-Path $bundle "SuspectFilters.txt") -Routing $routing

    # GUARANTEE: always write FilterClassification.json (even if suspects is empty)
    Write-JsonSafe -Path (Join-Path $bundle "FilterClassification.json") -Value $routing.Suspects

} catch {
    # If something truly fails, write a warning so we never silently miss it again
