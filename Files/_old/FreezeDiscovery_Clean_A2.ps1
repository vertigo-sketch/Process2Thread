...
"@
    try {
        ...
        # If Add-Type fails, we still keep GUID mappings.
    }
...
function Write-FiltersOnTargetVolumeArtifacts {
    ...
    # Return top 5 lines to inject into Ticket/Human summary
    $inject = @()
    if ($top.Count -gt 0) {
...
try {
    ...
    # A2 artifacts + top-5 lines for injection
...
$routing = [pscustomobject]@{ Queue=$routeQueue; Confidence=$routeConfidence; Reason=$routeReason }
...
if ($FilterSummaryLines -and $FilterSummaryLines.Count -gt 0) {
    $humanLines += ""
    $humanLines += $FilterSummaryLines
}

Write-Text -Path (Join-Path $bundle "Summary_Human.txt") -Content $humanLines

# Ticket summary (Tier-1 ready)
$ticketLines = @(
...
if ($FilterSummaryLines -and $FilterSummaryLines.Count -gt 0) {
    $ticketLines += ""
    $ticketLines += $FilterSummaryLines
}
