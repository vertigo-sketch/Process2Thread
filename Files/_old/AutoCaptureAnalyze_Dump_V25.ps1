...
Historical changes:
- V29: REMOVE ALL COMSVCS.DLL USAGE (blocked in environment).
       * ProcDump retry sequence: normal -> -64 -> -wer (switches supported by ProcDump) [2](https://icapitalnetwork-my.sharepoint.com/personal/jmiller_icapitalnetwork_com/_layouts/15/Doc.aspx?action=edit&mobileredirect=true&wdorigin=Sharepoint&DefaultItemOpen=1&sourcedoc={a8c8915f-6b87-4232-aba2-f8b09a805634}&wd=target(/Projects/process and handles.one/)&wdpartid={5276727a-0db4-44e0-a227-ca5b28bf705e}{1}&wdsectionfileid={fe5d4ba1-733f-4984-bc79-7a2c5154083e})
       * Fallback to native MiniDumpWriteDump via dbghelp.dll (no rundll32/comsvcs)
       * Skip UWP/WindowsApps/AppContainer-ish targets (e.g., Microsoft.AAD.BrokerPlugin) [3](https://icapitalnetwork-my.sharepoint.com/personal/jmiller_icapitalnetwork_com/Documents/Forms/DispForm.aspx?ID=23410&web=1)
#>
...
# Hard "never dump" list for known packaged/UWP-ish components you hit
$NeverDumpNames = @(
...
function Invoke-ProcDump {
    ...
    # -r = dump immediately (clone) [3](https://icapitalnetwork-my.sharepoint.com/personal/jmiller_icapitalnetwork_com/Documents/Forms/DispForm.aspx?ID=23410&web=1)
...
# ---------------------------
# Native fallback: MiniDumpWriteDump (dbghelp.dll) (NO comsvcs/rundll32)
# This is a suggested method (not from internal sources) to replace comsvcs.
# ---------------------------
function Ensure-MiniDumpNative {
    if ($script:MiniDumpNativeReady) { return }
...
"@
    ...
    try {
        ...
        throw ("Native MiniDump fallback not available (Add-Type failed): {0}" -f $_.Exception.Message)
    }
}
...
function Invoke-NativeMiniDump {
    ...
    $fs = $null
    try {
        # Best-effort: try All access first, then degrade
        ...
        $ok = [MiniDumpNative]::MiniDumpWriteDump($hProc, $TargetProcessId, $hFile, $dumpType, [IntPtr]::Zero, [IntPtr]::Zero, [IntPtr]::Zero)
        if (-not $ok) {
...
# ---------------------------
# Dump capture with ProcDump + Native fallback (NO comsvcs)
# ---------------------------
function Capture-Dump {
    ...
    # Attempt #2: ProcDump -64 (switch exists) [2](https://icapitalnetwork-my.sharepoint.com/personal/jmiller_icapitalnetwork_com/_layouts/15/Doc.aspx?action=edit&mobileredirect=true&wdorigin=Sharepoint&DefaultItemOpen=1&sourcedoc={a8c8915f-6b87-4232-aba2-f8b09a805634}&wd=target(/Projects/process and handles.one/)&wdpartid={5276727a-0db4-44e0-a227-ca5b28bf705e}{1}&wdsectionfileid={fe5d4ba1-733f-4984-bc79-7a2c5154083e})
    ...
    # Attempt #3: ProcDump -wer (switch exists) [2](https://icapitalnetwork-my.sharepoint.com/personal/jmiller_icapitalnetwork_com/_layouts/15/Doc.aspx?action=edit&mobileredirect=true&wdorigin=Sharepoint&DefaultItemOpen=1&sourcedoc={a8c8915f-6b87-4232-aba2-f8b09a805634}&wd=target(/Projects/process and handles.one/)&wdpartid={5276727a-0db4-44e0-a227-ca5b28bf705e}{1}&wdsectionfileid={fe5d4ba1-733f-4984-bc79-7a2c5154083e})
    ...
    # Attempt #4: Native MiniDumpWriteDump fallback (dbghelp.dll)
...
# MAIN LOOP
# ---------------------------
Write-Log "=== AutoCapture V29 START === ScriptRoot=$ScriptRoot DumpRoot=$DumpRoot DumpProfile=$DumpProfile"
Write-Status "AutoCapture V29 running (Admin OK). Logs: $LogFile" Cyan
