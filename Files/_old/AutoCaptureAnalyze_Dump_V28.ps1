...
- Analyzes dump with cdb; if -logo fails, falls back to stdout-captured analysis log
- Extracts "top blocked thread" section from cdb output (deterministic heuristic)
...
# ---------------------------
# ProcDump helpers (normal -> -64 -> -wer) [2](https://icapitalnetwork-my.sharepoint.com/personal/jmiller_icapitalnetwork_com/_layouts/15/Doc.aspx?action=edit&mobileredirect=true&wdorigin=Sharepoint&DefaultItemOpen=1&sourcedoc={a8c8915f-6b87-4232-aba2-f8b09a805634}&wd=target(/Projects/process and handles.one/)&wdpartid={5276727a-0db4-44e0-a227-ca5b28bf705e}{1}&wdsectionfileid={fe5d4ba1-733f-4984-bc79-7a2c5154083e})
# ---------------------------
...
# ---------------------------
# cdb analysis (two-pass: -logo then stdout capture)
# ---------------------------
function Analyze-Dump {
    ...
    $hex1 = Convert-ExitCodeHex -Code ([int]$p1.ExitCode)
    ...
    # Attempt 2: rerun WITHOUT -logo and capture stdout as the analysis log
    Write-Log "CDB START (capture) Dump=$DumpPath CaptureLog=$captureLog"
    $p2 = Start-Process -FilePath $CdbPath `
        -ArgumentList @('-z', $DumpPath, '-c', $cmd) `
        -WindowStyle Hidden `
        -PassThru `
        -Wait `
    ...
    $hex2 = Convert-ExitCodeHex -Code ([int]$p2.ExitCode)
    $capExists = (Test-Path $captureLog)
    $capHasText = $false
    if ($capExists) {
        try {
            $len = (Get-Item $captureLog).Length
            $capHasText = ($len -gt 10)
        } catch { $capHasText = $false }
    }

    Write-Log "CDB END (capture) ExitCode=$($p2.ExitCode) ExitHex=$hex2 CaptureExists=$capExists CaptureHasText=$capHasText StdErr=$cdbErr2"
    ...
    # Both failed: include excerpts for evidence
...
# MAIN LOOP
# ---------------------------
Write-Log "=== AutoCapture V32 START === ScriptRoot=$ScriptRoot DumpRoot=$DumpRoot DumpProfile=$DumpProfile"
Write-Status "AutoCapture V32 running (Admin OK). Logs: $LogFile" Cyan
