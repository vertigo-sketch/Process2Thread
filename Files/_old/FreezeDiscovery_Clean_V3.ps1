...
# ==========================================================
# Safe writers (fixes empty-string binding + ensures files exist)
# ==========================================================
function Write-Text {
    param(
        ...
        # Allow empty string / null / empty arrays without binding failures
        [Parameter(Mandatory=$true)]
        [AllowNull()]
        [AllowEmptyString()]
    ...
    # Sanitize each element to avoid empty-string issues downstream
...
function Write-JsonSafe {
    ...
    # Always write a JSON file even when Value is null or empty
    if ($null -eq $Value) {
...
function Get-FltmcRaw {
    ...
    # External command output can be 1 line or many; normalize to string[]
...
function Score-RoutingConfidence {
    ...
    [pscustomobject]@{ Score = $score; Label = $label }
}
...
# GUARANTEE: Always write routing & suspect artifacts
if ($null -eq $routing) {
