...
Purpose:
- Monitor all user-relevant processes
- Detect:
    1) UI hangs (Responding = False)
    2) True thread suspension (PsSuspend / SuspendThread)
- Capture dump immediately with ProcDump (-r)
- Analyze with cdb
- Provide live single-line console status
#>
...
if (-not $IsAdmin) {
    throw "This script MUST be run as Administrator. ProcDump cannot attach otherwise."
}
...
# START
# ==============================
Write-Log "=== AutoCapture V18 START ==="
Write-Host "AutoCapture running (Admin required). Ctrl+C to stop."
