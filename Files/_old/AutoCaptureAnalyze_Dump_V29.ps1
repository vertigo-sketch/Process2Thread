...
- If ProcDump fails, retries (-64) then (-wer); final fallback uses MiniDumpWriteDump (dbghelp.dll)
- Analyzes dump with cdb:
    * Uses -cf <scriptfile> (NOT -c "<cmd>") to avoid invalid-switch parsing and 0x80070057
    * Attempt with -logo; if log missing, fall back to stdout capture log
- Extracts "top blocked thread" section from cdb output
...
Historical changes:
- V33: Fix "cdb: Invalid switch" by using -cf script file instead of -c string (prevents -hang being parsed as a switch).
       Also fixes ExitHex formatting for negative HRESULT-style exit codes. [1](https://www.ionos.com/digitalguide/server/configuration/fixing-0x80070057-error/)
       Keeps ProcDump retries -64/-wer. [2](https://icapitalnetwork-my.sharepoint.com/personal/jmiller_icapitalnetwork_com/_layouts/15/Doc.aspx?action=edit&mobileredirect=true&wdorigin=Sharepoint&DefaultItemOpen=1&sourcedoc={a8c8915f-6b87-4232-aba2-f8b09a805634}&wd=target(/Projects/process and handles.one/)&wdpartid={5276727a-0db4-44e0-a227-ca5b28bf705e}{1}&wdsectionfileid={fe5d4ba1-733f-4984-bc79-7a2c5154083e})
#>
...
# ---------------------------
# ProcDump helpers (normal -> -64 -> -wer)
# ---------------------------
...
# ---------------------------
# Write CDB script file for -cf (avoids invalid switch parsing)
# ---------------------------
...
# ---------------------------
# CDB analysis (two-pass: -logo then stdout capture) USING -cf
# ---------------------------
function Analyze-Dump {
    ...
    # Attempt 1: preferred -logo
    Write-Log "CDB START (logo) Dump=$DumpPath Log=$cdbLog CmdFile=$cmdFile"
    $p1 = Start-Process -FilePath $CdbPath `
        -ArgumentList @('-z', $DumpPath, '-cf', $cmdFile, '-logo', $cdbLog) `
        -WindowStyle Hidden `
        -PassThru `
        -Wait `
    ...
    Write-Log "CDB END (logo) ExitCode=$($p1.ExitCode) ExitHex=$hex1 LogExists=$logExists StdOut=$cdbOut1 StdErr=$cdbErr1"
    ...
    # Attempt 2: rerun WITHOUT -logo and capture stdout into captureLog
    Write-Log "CDB START (capture) Dump=$DumpPath CaptureLog=$captureLog CmdFile=$cmdFile"
    $p2 = Start-Process -FilePath $CdbPath `
        -ArgumentList @('-z', $DumpPath, '-cf', $cmdFile) `
        -WindowStyle Hidden `
        -PassThru `
        -Wait `
    ...
    if ($capExists) {
        try {
            $capLen = (Get-Item $captureLog).Length
            $capHasText = ($capLen -gt 10)
        } catch { $capHasText = $false }
    }

    Write-Log "CDB END (capture) ExitCode=$($p2.ExitCode) ExitHex=$hex2 CaptureExists=$capExists CaptureHasText=$capHasText CaptureLen=$capLen StdErr=$cdbErr2"

    if ($capExists -and $capHasText) {
        return [pscustomobject]@{
            Success       = $true
    ...
    # Evidence excerpts
    $errEx1 = Get-FileExcerpt -Path $cdbErr1 -MaxLines 40
    $errEx2 = Get-FileExcerpt -Path $cdbErr2 -MaxLines 40
    if ($errEx1) { Write-Log ("CDB STDERR EXCERPT (logo):`n{0}" -f $errEx1) }
    if ($errEx2) { Write-Log ("CDB STDERR EXCERPT (capture):`n{0}" -f $errEx2) }
...
# MAIN LOOP
# ---------------------------
Write-Log "=== AutoCapture V33 START === ScriptRoot=$ScriptRoot DumpRoot=$DumpRoot DumpProfile=$DumpProfile"
Write-Status "AutoCapture V33 running (Admin OK). Logs: $LogFile" Cyan
...
do {
    ...
    if (-not $SinglePass) { Start-Sleep -Seconds $MonitorIntervalSeconds }
